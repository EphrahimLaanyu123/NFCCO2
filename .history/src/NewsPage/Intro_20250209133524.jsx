...
export default function Intro() {
  ...
  return (
    <div className="custom-news-container h-screen overflow-hidden relative" ref={container}>
      <motion.div style={{ y }} className="custom-motion-container relative h-full">
        <div className="custom-about-us-container">

          {/* Latest Article Section */}
          {latestArticle && (
            <section className="custom-latest-article my-10 md:w-1/2">
              ...
              <div className="custom-article-card">
                {latestArticle.fields.coverImage?.fields?.file?.url && (
                  <img
                    ...
                    className="custom-article-img w-full h-48 object-cover"
                  />
                )}
                <div className="custom-article-content p-4">
                  <h3 className="custom-article-title text-xl font-bold">{latestArticle.fields.blogTitle}</h3>
                  <p className="custom-article-date text-sm text-gray-700">{new Date(latestArticle.fields.createdDate).toLocaleDateString()}</p>
          ...
          {otherArticles.length > 0 && (
            <section className="custom-other-articles my-10 md:w-1/2">
              <h2 className="text-3xl font-bold mb-4">Other Latest Articles</h2>
              <div className="flex flex-wrap gap-6">
                {otherArticles.map((article) => (
                  <div key={article.sys.id} className="custom-article-card w-full md:w-1/3">
                    ...
                    <div className="custom-article-content p-4">
                      <h3 className="custom-article-title text-xl font-bold">{article.fields.blogTitle}</h3>
                      <p className="custom-article-date text-sm text-gray-700">{new Date(article.fields.createdDate).toLocaleDateString()}</p>
